import { SubjectAddFormController, SubjectAddFormModel } from "./SubjectAddForm.types";
 ...
 */
const getDefaultValues = (initialData?: SubjectAddFormModel) => {
    const defaultValues = {
        name: "",
        description: "",
        professorId: ""
 ...
 */
export const useSubjectAddFormController = (onSubmit?: () => void): SubjectAddFormController => {
    const { defaultValues, resolver } = useInitSubjectAddForm();
    const { redirectToLogin } = useAppRouter();
    const { addSubject: { mutation, key: mutationKey }, getSubjects: { key: queryKey } } = useSubjectApi();
    const { mutateAsync: add, status } = useMutation({
        mutationKey: [mutationKey], 
        mutationFn: mutation
    });
    const queryClient = useQueryClient();
    const submit = useCallback((data: SubjectAddFormModel) => // Create a submit callback to send the form data to the backend.
        add(data).then(() => {
    ...
    const {
        ...
        setValue,
        formState: { errors }
    } = useForm<SubjectAddFormModel>({ // Use the useForm hook to get callbacks and variables to work with the form.
