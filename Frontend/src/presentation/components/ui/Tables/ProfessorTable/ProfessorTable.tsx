...
import { useProfessorTableController } from "./ProfessorTable.controller";
import { ProfessorDTO } from "@infrastructure/apis/client";
 ...
 */
const useHeader = (): { key: keyof ProfessorDTO, name: string }[] => {
 ...
 */
const getRowValues = (entries: ProfessorDTO[] | null | undefined, orderMap: { [key: string]: number }) =>
    entries?.map(
        entry => {
            return {
                entry: entry,
...
export const ProfessorTable = () => {
    const { userId: ownProfessorId } = useAppSelector(x => x.profileReducer);
    const { formatMessage } = useIntl();
    const header = useHeader();
    const orderMap = header.reduce((acc, e, i) => { return { ...acc, [e.key]: i } }, {}) as { [key: string]: number }; // Get the header column order.
    const { handleChangePage, handleChangePageSize, pagedData, isError, isLoading, tryReload, labelDisplay, remove } = useProfessorTableController(); // Use the controller hook.
    const rowValues = getRowValues(pagedData?.data, orderMap); // Get the row values.

    return <DataLoadingContainer isError={isError} isLoading={isLoading} tryReload={tryReload}> {/* Wrap the table into the loading container because data will be fetched from the backend and is not immediately available.*/}
        <ProfessorAddDialog /> {/* Add the button to open the user add modal. */}
