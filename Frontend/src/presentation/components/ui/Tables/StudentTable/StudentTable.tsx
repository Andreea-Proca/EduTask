...
import { useStudentTableController } from "./StudentTable.controller";
import { StudentDTO } from "@infrastructure/apis/client";
import DeleteIcon from '@mui/icons-material/Delete';
import { StudentAddDialog } from "../../Dialogs/StudentAddDialog";
 ...
 */
const useHeader = (): { key: keyof StudentDTO, name: string }[] => {
 ...
 */
const getRowValues = (entries: StudentDTO[] | null | undefined, orderMap: { [key: string]: number }) =>
    entries?.map(
        entry => {
            return {
                entry: entry,
...
export const StudentTable = () => {
    const { userId: ownStudentId } = useAppSelector(x => x.profileReducer);
    const { formatMessage } = useIntl();
    const header = useHeader();
    const orderMap = header.reduce((acc, e, i) => { return { ...acc, [e.key]: i } }, {}) as { [key: string]: number }; // Get the header column order.
    const { handleChangePage, handleChangePageSize, pagedData, isError, isLoading, tryReload, labelDisplay, remove } = useStudentTableController(); // Use the controller hook.
    const rowValues = getRowValues(pagedData?.data, orderMap); // Get the row values.

    return <DataLoadingContainer isError={isError} isLoading={isLoading} tryReload={tryReload}> {/* Wrap the table into the loading container because data will be fetched from the backend and is not immediately available.*/}
        <StudentAddDialog /> {/* Add the button to open the user add modal. */}
                ...
                <TableBody>
                    {
                        rowValues?.map(({ data, entry }, rowIndex) => <TableRow key={`row_${rowIndex + 1}`}>
                            ...
                            <TableCell> {/* Add other cells like action buttons. */}
                                {entry.id !== ownStudentId && <IconButton color="error" onClick={() => remove(entry.id || '')}>
                                    <DeleteIcon color="error" fontSize='small' />
                                </IconButton>}
                                {entry.id !== ownStudentId &&  <UserEditDialog id={entry.id ?? ''}/>}
                            </TableCell>
                        </TableRow>)
                    }
                </TableBody>
